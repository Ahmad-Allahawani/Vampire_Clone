...
var spr_jav_reg = preload("res://ASS/spear/spear_07.png")
var spr_jav_atk = preload("res://ASS/spear/spear_08.png")
...
func _physics_process(delta):
	if target_array.size()>0:
		...
		rotation = global_position.direction_to(player.global_position).angle() + deg_to_rad(100)
...
func process_path():
	...
	var new_rotation_degrees = angle.angle() + deg_to_rad(100)
...
func _on_reset_pos_timer_timeout():
	var chose_direction = randi()% 4
	reset_pos = player.global_position
	match chose_direction:
		0:
			reset_pos.x += 50
		1:
			reset_pos.x -= 50
